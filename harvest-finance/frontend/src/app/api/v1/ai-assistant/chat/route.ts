...
function generateAIResponse(message: string, context?: FarmContext, history?: ChatMessage[]): ChatResponse {
  ...
  if (lowerMessage.includes('crop') || lowerMessage.includes('plant')) {
    if (context?.currentSeason) {
      ...
      if (context.currentSeason.toLowerCase().includes('summer')) {
        ...
        response += 'Root vegetables like carrots, potatoes, and onions are excellent choices.';
      } else {
        response += 'Seasonal crops will give you the best returns on your farming investment.';
      }
    } else {
      response = 'Different crops perform better in different seasons. What season are you planning for?';
    }
    suggestions = [
  ...
  } else if (lowerMessage.includes('vault') || lowerMessage.includes('deposit') || lowerMessage.includes('yield')) {
    if (context?.vaultBalance !== undefined) {
      ...
      response = 'Vaults are a great way to earn passive income on your crypto assets. Start with smaller deposits to test the waters.';
    }
    suggestions = [
  ...
  } else if (lowerMessage.includes('milestone') || lowerMessage.includes('goal')) {
    if (context?.currentMilestone) {
      response = `Congratulations on reaching the ${context.currentMilestone} milestone! `;
      response += 'This is a significant achievement. Consider setting your next target to keep the momentum going.';
    } else {
      response = 'Milestones help track your farming progress. Set achievable goals and celebrate each accomplishment.';
    }
    suggestions = [
    ...
    response = 'Seasonal planning is crucial for successful farming. Each season brings different opportunities and challenges. Monitor weather patterns and adjust your strategy accordingly.';
    suggestions = [
    ...
    response = 'I\'m here to help with all aspects of your farming journey! Whether it\'s crop selection, vault strategies, seasonal planning, or milestone tracking, I can provide personalized advice based on your current situation.';
    suggestions = [
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      existing.push({
        user: { content: message, timestamp: new Date().toISOString(), context },
