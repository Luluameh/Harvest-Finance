  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <CardBody>
          <Stack gap="md">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-base font-semibold text-gray-800">User Growth (Last 12 Months)</h3>
            </div>
            <ResponsiveContainer width="100%" height={260}>
              <LineChart data={userGrowth} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
        ...
        <CardBody>
          <Stack gap="md">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-base font-semibold text-gray-800">Deposit & Withdrawal Trends (Last 12 Months)</h3>
            </div>
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={depositWithdrawTrends} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                <XAxis dataKey="period" tick={{ fontSize: 12 }} />
                ...
                <Bar dataKey="withdrawals" name="Withdrawals" fill="#2563eb" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </Stack>
        </CardBody>
        ...
        <CardBody>
          <Stack gap="md">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-base font-semibold text-gray-800">Vault Usage Distribution</h3>
            </div>
            {pieData.length > 0 ? (
              <ResponsiveContainer width="100%" height={260}>
                <PieChart>
                  ...
                  >
                    {pieData.map((_, i) => (
                      <Cell key={i} fill={VAULT_COLORS[i % VAULT_COLORS.length]} />
                    ))}
                  </Pie>
                  <Tooltip />
                </PieChart>
      ...
      <Card className="border-none shadow-md">
        <CardBody>
          <Stack gap="md">
            <h3 className="text-base font-semibold text-gray-800">Vault Deposits by Type</h3>
                ...
                vaultDistribution.map((v, i) => (
                  <div key={v.type} className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <span className="text-sm text-gray-700">{VAULT_LABELS[v.type] ?? v.type}</span>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-semibold text-gray-900">{formatCurrency(v.totalDeposits)}</p>
                      <p className="text-xs text-gray-400">{v.count} vault{v.count !== 1 ? 's' : ''}</p>
                    </div>
                  </div>
                ))
              ) : (
